...
MySQL_Monitor::MySQL_Monitor() {
	...
	pthread_mutex_init(&ping_log_mutex,NULL);
...
__exit_monitor_ping_thread:
	mmsd->t2=monotonic_time();
	{
		pthread_mutex_lock(&GloMyMon->ping_log_mutex);
...
//		if ((rand() % 10) ==0) {
		...
		pthread_mutex_unlock(&GloMyMon->ping_log_mutex);
#ifdef TEST_AURORA
//		}
#endif // TEST_AURORA
	}
...
__end_monitor_ping_loop:
		if (mysql_thread___monitor_enabled==true) {
			pthread_mutex_lock(&ping_log_mutex);
			...
			pthread_mutex_unlock(&ping_log_mutex);
		}
